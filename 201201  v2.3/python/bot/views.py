        ...
        for event in events:
            if isinstance(event, MessageEvent):
                if isinstance(event.message, TextMessage):
                    ...
                    if mtext=="00是，開始登錄":
                    ...
                    elif re.match(r"^(05月[0-9]).+", mtext) or re.match(r"^(05/[0-9]).+", mtext) or re.match(r"^(05\.[0-9]).+", mtext):
                        userid=str(event.source).split('"')[7]
                    ...
                    elif re.match(r"^(05)\.?$", mtext) or re.match(r"^(06)\.?$", mtext) or re.match(r"^(11)\.?$", mtext) or re.match(r"^(12)\.?$", mtext) or re.match(r"^(13)\.?$", mtext) or re.match(r"^(14)\.?$", mtext):
                        line_bot_api.reply_message(event.reply_token, TextSendMessage(
                    ...
                    elif re.match(r"^(06月[0-9]).+", mtext) or re.match(r"^(06/[0-9]).+", mtext) or re.match(r"^(06\.[0-9]).+", mtext):
                        userid=str(event.source).split('"')[7]
                        time=str(event.timestamp)[0:10]
                        db=pymysql.connect(host="***", user="***", passwd="***", db="***")
                        cursor=db.cursor()
                        ...
                        sql_str2="select week(from_unixtime(time)) from forward where userid=\'{}\' order by time desc limit 1".format(userid)
                        cursor.execute(sql_str2)
                        sql_str3="select count(*) from forward where userid=\'{}\' and week(from_unixtime(time))=\'{}\'".format(userid, str(cursor.fetchone()).split("(")[1].split(",")[0])
                        cursor.execute(sql_str3)
                        db.close()
                        count=str(cursor.fetchone()).split("(")[1].split(",")[0]
                        if count=="2":
                            messageQ07_event(event)
                        else:
                            line_bot_api.reply_message(event.reply_token, TextSendMessage(
                    ...
                    elif mtext=="07少於20則" or mtext=="0720-100則" or mtext=="07100-500則" or mtext=="07500則以上":
                        userid=str(event.source).split('"')[7]
                        db=pymysql.connect(host="***", user="***", passwd="***", db="***")
                        cursor=db.cursor()
                        db.autocommit(True)
                        sql_str="update forward set q7=\"{}\" where userid=\"{}\" order by time desc limit 1".format(mtext, userid)
                        cursor.execute(sql_str)
                        db.close()
                        messageQ08_event(event)
                        return HttpResponse("")

                    elif mtext=="08少於20則" or mtext=="0820-100則" or mtext=="08100-500則" or mtext=="08500則以上":
                        userid=str(event.source).split('"')[7]
                        db=pymysql.connect(host="***", user="***", passwd="***", db="***")
                        cursor=db.cursor()
                        db.autocommit(True)
                        sql_str="update forward set q8=\"{}\" where userid=\"{}\" order by time desc limit 1".format(mtext, userid)
                        cursor.execute(sql_str)
                        db.close()
                        line_bot_api.reply_message(event.reply_token, TextSendMessage(
                            text="已完成登錄，宗教機器人謝謝您！下次看到有感的訊息，別忘了轉傳給我喔！"
                    ...
                    elif re.match(r"^(11月[0-9]).+", mtext) or re.match(r"^(11/[0-9]).+", mtext) or re.match(r"^(11\.[0-9]).+", mtext):
                        userid=str(event.source).split('"')[7]
                        time=str(event.timestamp)[0:10]
                        db=pymysql.connect(host="***", user="***", passwd="***", db="***")
                        cursor=db.cursor()
                    ...
                    elif re.match(r"^(12月[0-9]).+", mtext) or re.match(r"^(12/[0-9]).+", mtext) or re.match(r"^(12\.[0-9]).+", mtext):
                        userid=str(event.source).split('"')[7]
                        time=str(event.timestamp)[0:10]
                        db=pymysql.connect(host="***", user="***", passwd="***", db="***")
                        cursor=db.cursor()
                        ...
                        cursor=db.cursor()
                        db.autocommit(True)
                        sql_str="insert into ig(userid, time) values(\"{}\",\"{}\")".format(userid, time)
                        cursor.execute(sql_str)
                        db.close()
                        line_bot_api.reply_message(event.reply_token, TextSendMessage(
                            text="14.請問您是在誰的IG發文？（請先輸入14，再輸入人名或社團名稱，例如：14王小明、14媽祖進香群；若是在自己的IG發文，請輸入：14自己）"
                        ))
                        return HttpResponse("")

                    elif re.match(r"^(14).+",mtext):
                        userid=str(event.source).split('"')[7]
                        db=pymysql.connect(host="***", user="***", passwd="***", db="***")
                        cursor=db.cursor()
                        db.autocommit(True)
                        sql_str="update ig set q1=\"{}\" where userid=\"{}\" order by time desc limit 1".format(mtext, userid)
                        cursor.execute(sql_str)
                        db.close()
                        line_bot_api.reply_message(event.reply_token, TextSendMessage(
                            text="已完成登錄，宗教機器人謝謝您！下次若還有主動發文，別忘了來這裡登錄喔！"
                    ...
                    filename="C:/***/uploads/"+message_id+".jpg"
                    ...
                    cursor=db.cursor()
                    db.autocommit(True)
                    sql_str="insert into forward(userid, time, content) values(\"{}\",\"{}\",\"{}\")".format(userid, time, "圖片-"+message_id)
                    cursor.execute(sql_str)
                    db.close()
                    messageQ00_event(event)
                    return HttpResponse("")
                    ...
                    message_id=str(event.message.id)
                    filename="C:/***/uploads/"+message_id+".m4a"
                    message_content=line_bot_api.get_message_content(message_id)
                    with open(filename, "wb") as tf:
                        for chunk in message_content.iter_content():
                            tf.write(chunk)
                    ...
                    cursor=db.cursor()
                    db.autocommit(True)
                    sql_str="insert into forward(userid, time, content) values(\"{}\",\"{}\",\"{}\")".format(userid, time, "聲音-"+message_id)
                    cursor.execute(sql_str)
                    db.close()
                    messageQ00_event(event)
                    return HttpResponse("")
                    ...
                    message_id=str(event.message.id)
                    filename="C:/***/uploads/"+message_id+".mp4"
                    message_content=line_bot_api.get_message_content(message_id)
                    with open(filename, "wb") as tf:
                        for chunk in message_content.iter_content():
                            tf.write(chunk)
                    ...
                    cursor=db.cursor()
                    db.autocommit(True)
                    sql_str="insert into forward(userid, time, content) values(\"{}\",\"{}\",\"{}\")".format(userid, time, "影片-"+message_id)
                    cursor.execute(sql_str)
                    db.close()
                    messageQ00_event(event)
                    return HttpResponse("")
...
def messageQ07_event(event):
    messageQ07=TemplateSendMessage(
        alt_text="Buttons Template",
        template=ButtonsTemplate(
            text="7.請問您昨天一整天，大約瀏覽過幾則Line的訊息？",
            actions=[
                MessageTemplateAction(label="少於20則", text="07少於20則"),
                MessageTemplateAction(label="20-100則", text="0720-100則"),
                MessageTemplateAction(label="100-500則", text="07100-500則"),
                MessageTemplateAction(label="500則以上", text="07500則以上")
            ]
        )
    )
    line_bot_api.reply_message(event.reply_token, messageQ07)

def messageQ08_event(event):
    messageQ08=TemplateSendMessage(
        alt_text="Buttons Template",
        template=ButtonsTemplate(
            text="8.請問您昨天一整天，大約瀏覽過幾則FB的貼文？",
            actions=[
                MessageTemplateAction(label="少於20則", text="08少於20則"),
                MessageTemplateAction(label="20-100則", text="0820-100則"),
                MessageTemplateAction(label="100-500則", text="08100-500則"),
                MessageTemplateAction(label="500則以上", text="08500則以上")
            ]
        )
    )
    line_bot_api.reply_message(event.reply_token, messageQ08)
